...
export async function generateSupplierLedgerPDF(supplier, ledgerEntries, stats, settings) {
  ...
  doc.setFont('helvetica', 'bold');
  doc.setTextColor(0, 0, 0);
  doc.text('SUPPLIER LEDGER', centerX, y, { align: 'center' });
  y += 4;
  doc.setDrawColor(0, 0, 0);
  ...
  doc.setFont('helvetica', 'normal');
  doc.text(supplier.supplier_name || '-', leftX + 25, y);

  doc.setFont('helvetica', 'bold');
  doc.text('Mobile:', leftX, y + 6);
  doc.setFont('helvetica', 'normal');
  doc.text(supplier.mobile_no || '-', leftX + 25, y + 6);

  // Right side - Summary Stats
  doc.setFont('helvetica', 'bold');
  doc.text('Total Purchases:', rightX - 60, y);
  doc.setFont('helvetica', 'normal');
  doc.text(formatCurrency(stats.totalPurchases), rightX, y, { align: 'right' });
  ...
  autoTable(doc, {
    ...
    willDrawPage: function (data) {
      if (data.pageNumber > 1) {
        ...
        doc.text('Supplier Ledger', margin, topY);
        doc.text(`${supplier.supplier_name}`, pageWidth - margin, topY, { align: 'right' });
  ...
  if (finalY < pageHeight - 50) {
    ...
    doc.setFont('helvetica', 'bold');
    doc.text('Total Purchases:', summaryX + 5, summaryY + 8);
    doc.setFont('helvetica', 'bold');
    doc.text(formatCurrency(stats.totalPurchases), summaryX + 65, summaryY + 8, { align: 'right' });
...
/**
 * View supplier ledger PDF in new window
 */
export async function viewSupplierLedgerPDF(supplier, ledgerEntries, stats, settings) {
  try {
