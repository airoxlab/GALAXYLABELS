...
export async function generateCustomerLedgerPDF(customer, ledgerEntries, stats, settings) {
  ...
  doc.setFont(undefined, 'normal');
  doc.text(customer.customer_name || '-', margin + 30, yPos);

  doc.setFont(undefined, 'bold');
  doc.text('Mobile:', margin, yPos + 5);
  doc.setFont(undefined, 'normal');
  doc.text(customer.mobile_no || '-', margin + 30, yPos + 5);

  // Right column - Stats
  const rightCol = pageWidth - margin - 60;
  doc.setFont(undefined, 'bold');
  doc.text('Total Sales:', rightCol, yPos);
  doc.setFont(undefined, 'normal');
  doc.text(formatCurrency(stats.totalSales), rightCol + 30, yPos);

  doc.setFont(undefined, 'bold');
  doc.text('Total Paid:', rightCol, yPos + 5);
  doc.setFont(undefined, 'normal');
  doc.text(formatCurrency(stats.totalPayments), rightCol + 30, yPos + 5);

  doc.setFont(undefined, 'bold');
  doc.text('Balance:', rightCol, yPos + 10);
  doc.setFont(undefined, 'normal');
  doc.text(formatCurrency(stats.outstandingBalance), rightCol + 30, yPos + 10);
  ...
  doc.setTextColor(...primaryColor);
  doc.text(formatCurrency(stats.totalSales), pageWidth - margin - 5, finalY + 8, { align: 'right' });

  doc.setTextColor(...grayColor);
  doc.text('Total Credit:', pageWidth - margin - 75, finalY + 16);
  doc.setTextColor(...primaryColor);
  doc.text(formatCurrency(stats.totalPayments), pageWidth - margin - 5, finalY + 16, { align: 'right' });

  doc.setFont(undefined, 'bold');
  doc.setTextColor(...primaryColor);
  doc.text('Net Balance:', pageWidth - margin - 75, finalY + 26);
  doc.text(
