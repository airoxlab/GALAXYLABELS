...
import { downloadPurchaseOrderPDF, generatePurchasesReportPDF, generatePurchaseOrderPDFBase64 } from '@/components/purchases/PurchasePDF';
import { usePermissions } from '@/hooks/usePermissions';
import { sendPurchaseInvoiceWhatsApp, isWhatsAppAvailable } from '@/lib/whatsapp';
...
export default function PurchasesPage() {
  ...
  async function handleShareWhatsApp(po) {
    if (!isWhatsAppAvailable()) {
      notify.error('WhatsApp is only available in the desktop app');
      return;
    }

    if (!po.suppliers?.mobile_no) {
      notify.error('Supplier does not have a phone number');
      return;
    }

    setSendingWhatsApp(po.id);
    ...
    try {
      ...
      if (settings?.whatsapp_attach_invoice_image !== false) {
        pdfBase64 = await generatePurchaseOrderPDFBase64(po, items || [], settings);
      }
      ...
      await sendPurchaseInvoiceWhatsApp({
        ...
        pdfBase64,
        onSuccess: () => {
          notify.success('Purchase order sent via WhatsApp!');
        },
        onError: (error) => {
          notify.error(error);
        },
                          ...
                          <div className="relative">
                            <button
                              onClick={(e) => {
                                ...
                                setOpenShareMenu(openShareMenu?.id === po.id ? null : { id: po.id, po, top: rect.bottom + 4, right: window.innerWidth - rect.right });
                                setOpenActionMenu(null);
                              }}
                              disabled={sendingWhatsApp === po.id}
                              className={cn(
                                "p-1.5 rounded-lg transition-all",
                                sendingWhatsApp === po.id
                          ...
                          <div className="relative">
                            <button
                              onClick={(e) => {
                                e.stopPropagation();
                                const rect = e.currentTarget.getBoundingClientRect();
                                setOpenActionMenu(openActionMenu === po.id ? null : { id: po.id, top: rect.bottom + 4, right: window.innerWidth - rect.right });
                              }}
      ...
      {openShareMenu && (
        <>
          ...
          >
            <button
              onClick={() => {
                handleShareWhatsApp(openShareMenu.po);
              }}
      ...
      {openActionMenu && (
        <>
          ...
          >
            <button
              onClick={() => {
                const po = currentOrders.find(p => p.id === openActionMenu.id);
                if (po) handleViewDetails(po);
                setOpenActionMenu(null);
              }}
