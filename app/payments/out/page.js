...
export default function PaymentOutPage() {
  ...
  async function generateVoucherNo() {
    ...
    if (data && data.length > 0) {
      const lastNo = parseInt(data[0].voucher_no.split('-')[1]);
      setFormData(prev => ({ ...prev, voucher_no: `PV-${String(lastNo + 1).padStart(4, '0')}` }));
    } else {
      setFormData(prev => ({ ...prev, voucher_no: 'PV-0001' }));
    }
  }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await supabase.from('supplier_ledger').insert([{
        ...
        reference_no: formData.voucher_no,
        debit: 0,
        credit: amount,
        balance: newBalance,
